...
RUN apt-get update && \
    ...
    env DEBIAN_FRONTEND=noninteractive apt-get install -y \
      ...
      sudo \
      lsb-release \
      curl sudo wget pluma meld mate-calc atril pulseaudio vim

RUN curl -fSL "https://download.nomachine.com/download/8.1/Linux/nomachine_8.1.2_1_amd64.deb" -o nomachine.deb \
